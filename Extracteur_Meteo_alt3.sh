...
if ! curl -s "wttr.in/$VILLE?format=j1" -o "$FICHIER_METEO"; then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Erreur : Impossible de se connecter à wttr.in pour la ville de $VILLE." >> "$FICHIER_LOG"
...
if grep -q "Unknown location" "$FICHIER_METEO" || [ ! -s "$FICHIER_METEO" ]; then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Erreur : La ville $VILLE est inconnue ou les données sont manquantes." >> "$FICHIER_LOG"
...
if [ -z "$TEMPERATURE_ACTUELLE" ] || [ -z "$TEMPERATURE_DEMAIN" ] || [ -z "$VITESSE_VENT" ] || [ -z "$HUMIDITE" ] || [ -z "$VISIBILITE" ]; then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Erreur : Données météorologiques non valides pour la ville de $VILLE." >> "$FICHIER_LOG"
